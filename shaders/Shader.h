...
public:
	CShader();
	virtual		~CShader();
	bool		CreateShaderProgram(std::string stName, const char* strVertexShaderFileName, const char* strFragmentShaderFileName);
...
// Operations
private:
	std::string		LoadFileData(const char* stFileName);
	GLuint			LoadShaderFromFile(const char* stFileName, GLenum uiEnumType);
