...
void Font::make_dlist( FT_Face face, char ch) {
	...
	col[0] = 1.0; col[1] = 0.0; col[2] = 0.0; //r,g,b values for each vertex
	col[3] = 1.0; col[4] = 0.0; col[5] = 0.0;
	col[6] = 1.0; col[7] = 0.0; col[8] = 0.0;
	col[9] = 1.0; col[10] = 0.0; col[11] = 0.0; 
	col[12] = 1.0; col[13] = 0.0; col[14] = 0.0;
	col[15] = 1.0; col[16] = 0.0; col[17] = 0.0;
	...
	GLint texLocation= glGetAttribLocation(myShader->GetProgramObjID(), "in_TexCoord");
...
void Font::loadShader(string filename)
{
	...
	if(!myShader->CreateShaderProgram(filename, filenameVert.c_str(), filenameFrag.c_str()))
	{
		std::cout << "failed to load shader" << std::endl;
	}		
}

void Font::init(const char * fname, unsigned int h) {
...
void print(glm::mat4 &ProjectionMatrix, Font &ft_font, float x, float y, const char *fmt, ...)  {
	...
	glUseProgram(Font::myShader->GetProgramObjID());
	glUniformMatrix4fv(glGetUniformLocation(Font::myShader->GetProgramObjID(), "ProjectionMatrix"), 1, GL_FALSE, &ProjectionMatrix[0][0]);

	glUniform1i(glGetUniformLocation(Font::myShader->GetProgramObjID(), "DiffuseMap"), 0);
	...
	for(int i=0;i<lines.size();i++) {
		...
		for(int k=0; k<lines[i].length(); k++)
		{
			...
			glUniformMatrix4fv(glGetUniformLocation(Font::myShader->GetProgramObjID(), "ModelViewMatrix"), 1, GL_FALSE, &t[0][0]);
